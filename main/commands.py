...
from passive_enumeration.passiveDomain import run_crtsh, run_subfinder, run_shodan, run_abuseipdb, run_assetfinder, run_rapiddns, run_wayback
from passive_enumeration.passiveSubdomain import run_sub_crtsh, run_sub_subfinder, run_sub_shodan, run_sub_abuseipdb, run_sub_assetfinder, run_sub_rapiddns, run_sub_wayback
from dns_buteforcing.dnsbrute import run_4char, run_assetnote, run_chaos, run_jhaddix, run_nokovo
from scanning.scanURL import run_nuclei
from scanning.scanIP import run_nuclei_ip
...
def enumerate_subdomains(subdomains, enum_types):
    for enum_type in enum_types:
        if enum_type == 'subfinder':
            # Call subfinder enumeration function
            run_sub_subfinder(subdomains)
        elif enum_type == 'crtsh':
            # Call crtsh enumeration function
            run_sub_crtsh(subdomains)
        elif enum_type == 'wayback':
            # Call wayback enumeration function
            run_sub_wayback(subdomains)
        elif enum_type == 'assetfinder':
            # Call assetfinder enumeration function
            run_sub_assetfinder(subdomains)
        elif enum_type == 'rappiddns':
            # Call rapiddns enumeration function
            run_sub_rapiddns(subdomains)
        elif enum_type == 'abuseipdb':
            # Call abuseipdb enumeration function
            run_sub_abuseipdb(subdomains)
        elif enum_type == 'shodan':
            # Call shodan enumeration function
            run_sub_shodan(subdomains)
        else:
            print(f"Unknown enumeration type: {enum_type}")
...
def list_subdomains(subdomain='*', domain='*', program='*', sources=None, scope=None, resolved=None, brief=False, source_only=False,
                    ...
                    stats_program=False, stats_domain=False, stats_created_at=False, stats_updated_at=False, passive=None):
    ...
    if passive:
        ...
        # If 'all' is specified, use all_enum_types; otherwise, use the provided types
        enum_types = all_enum_types if passive == ['all'] else passive
        enumerate_subdomains(subdomains, enum_types)
        return
