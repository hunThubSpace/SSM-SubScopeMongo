...
from passive.subfinder import run_subfinder
from passive.wayback import run_wayback
from passive.abuseipdb import run_abuseipdb
from passive.crtsh import run_crtsh
from passive.rapiddns import run_rapiddns
from passive.shodan import run_shodan
from passive.assetfinder import run_assetfinder
from dnsbrute.fourchar import run_4char
from dnsbrute.assetnote import run_assetnote
from dnsbrute.chaos import run_chaos
from dnsbrute.jhaddix import run_jhaddix
from dnsbrute.nokovo import run_nokovo
from resolving.resolver import run_resolve
from permutation.perm import run_perm
from scanning.url_scanning import run_nuclei_url
from scanning.ip_scanning import run_nuclei_ip
...
def enumerate_domains(domains, enum_types):
    for enum_type in enum_types:
        if enum_type == 'subfinder':
            # Call subfinder enumeration function
            run_subfinder(domains)
        elif enum_type == 'crtsh':
            # Call crtsh enumeration function
            run_crtsh(domains)
        elif enum_type == 'wayback':
            # Call wayback enumeration function
            run_wayback(domains)
        elif enum_type == 'assetfinder':
            # Call assetfinder enumeration function
            run_assetfinder(domains)
        elif enum_type == 'rappiddns':
            # Call rapiddns enumeration function
            run_rapiddns(domains)
        elif enum_type == 'abuseipdb':
            # Call abuseipdb enumeration function
            run_abuseipdb(domains)
        elif enum_type == 'shodan':
            # Call shodan enumeration function
            run_shodan(domains)
        else:
            print(f"Unknown enumeration type: {enum_type}")
            
def dnsbruteforce(domains, wordlists):
    for wordlist in wordlists:
        if wordlist == '4char':
            # Call 4char enumeration function
            run_4char(domains)
        elif wordlist == 'assetnote':
            # Call assetnote enumeration function
            run_assetnote(domains)
        elif wordlist == 'chaos':
            # Call chaos enumeration function
            run_chaos(domains)
        elif wordlist == 'jhaddix':
            # Call jhaddix enumeration function
            run_jhaddix(domains)
        elif wordlist == 'nokovo':
            # Call nokovo enumeration function
            run_nokovo(domains)
        else:
            print(f"Unknown enumeration type: {wordlist}")

def list_domains(domain='*', program='*', brief=False, count=False, scope=None, passive=None, dnsbrute=None):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")  # For potential logging
    ...
    # Handle enumeration based on the enum parameter
    if passive:
        display_output = 0
        # Define all available enumeration types
        all_enum_types = ['crtsh', 'subfinder', 'wayback', 'assetfinder', 'rappiddns', 'abuseipdb', 'shodan']
        
        # If 'all' is specified, use all_enum_types; otherwise, use the provided types
        enum_types = all_enum_types if passive == ['all'] else passive
        enumerate_domains(domains, enum_types)

    # Handle dnsbrute based on the wordlists parameter
    if dnsbrute:
        display_output = 0
        # Define all available enumeration types
        all_wordlists = ['4char', 'assetnote', 'chaos', 'jhaddix', 'nokovo']
        
        # If 'all' is specified, use all_wordlists; otherwise, use the provided types
        wordlists = all_wordlists if dnsbrute == ['all'] else dnsbrute
        dnsbruteforce(domains, wordlists)
...
def list_subdomains(subdomain='*', domain='*', program='*', sources=None, scope=None, resolved=None, brief=False, source_only=False,
                    ...
                    stats_program=False, stats_domain=False, stats_created_at=False, stats_updated_at=False, resolve=None, permutation=None):
    ...
    # Handle resolve based on the wordlists parameter
    if resolve:
        display_output = 0
        if domain == "*":
            seen_domains = set()
            for d in filtered_subdomains:
                if d['domain'] not in seen_domains:
                    seen_domains.add(d['domain'])
            for i in seen_domains:
                subs = []
                for s in filtered_subdomains:
                    if s["domain"] == i:
                        subs.append(s["subdomain"])
                #print(subs)
                run_resolve(i, subs)
                subs.remove
        else:
            ...
            for s in subdomains:
                subs.append(s["subdomain"])
            run_resolve(domain, subs)
            
    # Handle permutation based on the wordlists parameter
    if permutation:
        display_output = 0
        if domain == "*":
            seen_domains = set()
...
def scanning_urls(urls, types):
    for type in types:
        if type == 'nuclei':
            # Call nuclei function
            run_nuclei_url(urls)
        else:
...
def list_urls(url='*', subdomain='*', domain='*', program='*', scheme=None, method=None, port=None,
               ...
               flag=None, content_length=None, path=None, stats_flag=None, stats_content_length=None, stats_path=None, scan=None):
    ...
    if scan:
        ...
        # If 'all' is specified, use all_enum_types; otherwise, use the provided types
        types = all_types if scan == ['all'] else scan
        scanning_urls(live_urls, types)
...
def list_ip(ip='*', program='*', cidr=None, asn=None, port=None, hostname=None, domain=None, 
            ...
            stats_os=False, stats_product=False, stats_version=False, stats_cves=False, scan=None):
