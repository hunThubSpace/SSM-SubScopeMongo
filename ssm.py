...
from importExport.importExport import importdb, exportdb
from convert.convert import json_to_csv, json_to_xml, bson_to_csv, bson_to_xml
    ...
    if args.command == 'program':
        ...
        if args.action == 'add':
            ...
            list_subdomains(subdomain=args.subdomain, domain=args.domain, program=args.program, sources=args.source,
                            ...
                            stats_program=args.stats_program, stats_created_at=args.stats_created_at, stats_updated_at=args.stats_updated_at, passive=args.passive)
        ...
        if args.format == 'csv':
            ...
            if file_extension == '.json':
                ...
                bson_to_csv(bson_file=args.input, csv_file=args.output)
            
        elif args.format == 'xml':
            _, file_extension = os.path.splitext(args.input)
            if file_extension == '.json':
                json_to_xml(json_file=args.input, xml_file=args.output)
            elif file_extension == '.bson':
                bson_to_xml(bson_file=args.input, xml_file=args.output)
